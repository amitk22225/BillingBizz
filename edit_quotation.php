...
$query = "select * from quotation where SID ='{$_GET["id"]}'";
...
$query_products = "select * from quotation_products where SID ='{$_GET["id"]}'";
...
<div class='container'>
    <div class="gst">
        ...
        <div class = "sentence"> <p> Edit Quotation </p></div>
    </div>
    <?php
  ...
  if ($con->query($sql)) {
    ...
    for ($i = 0; $i < count($_POST["pname"]); $i++) {
        ...
        $check_query = "SELECT * FROM quotation_products WHERE SID = '{$sid}' AND PNAME = '{$pname}'";
        $check_result = mysqli_query($con, $check_query);

        if ($check_result && mysqli_num_rows($check_result) > 0) {
            // Product exists, perform update
            $update_query = "UPDATE quotation_products SET PRICE='{$price}', QTY='{$qty}', sgst='{$sgst}', cgst='{$cgst}', TOTAL='{$total}' WHERE SID = '{$sid}' AND PNAME = '{$pname}'";
            ...
            $insert_query = "INSERT INTO quotation_products (SID, PNAME, hsn, PRICE, QTY, sgst, cgst, TOTAL) VALUES ('{$sid}', '{$pname}', '{$hsn}', '{$price}', '{$qty}', '{$sgst}', '{$cgst}', '{$total}')";
    ...
    echo "<script>
        ...
        echo "<div class='alert alert-danger'>Failed to update Quotation products.</div>";
    }
} else {
    echo "<div class='alert alert-danger'>Quotation Update Failed.</div>";
}
    }
    ?>
    <form method='post' action="" autocomplete='off' >
        <div class='row'>
            <div class='col-md-4'>
                <h5 class='text-success'>Quotation Details</h5>
            ...
            <div class='col-md-4'>
                <h5 class='text-success'>Customer/Client Details</h5>
                <div class='form-group'>
                    <label>Name</label>
        ...
        <div class='col-md-12'>
            ...
            <textarea class="form-control" name="tandc" id="tandc" rows="4" cols="40"><?php echo $tandc; ?></textarea>
            </div>
            <input type='submit' name='submit' value='Update Quotation' class='btn btn-success float-right'>
        </div>
    </form>
</div>
<script>
    ...
    $.ajax({
        url: 'delete_product_quotation.php',
        type: 'POST',
