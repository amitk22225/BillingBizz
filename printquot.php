<?php 
 ...
 if($res1->num_rows>0){
  ...
  $sql="select * from quotation where SID='{$_GET["id"]}'";
  $res=$con->query($sql);
  if($res->num_rows>0){
	  $row=$res->fetch_assoc();
  ...
  $sql="select * from quotation_products where SID='{$_GET["id"]}'";
  $res=$con->query($sql);
  if($res->num_rows>0){
  ...
  $filepath= "uploads/";
  $filename= $info["customer"].'_quotation.pdf';
  
  if($_GET['ACTION']=='EMAIL'){
    
    header("location: records-quotation.php");
    ...
    $pdf->Output($filepath.$filename,'F');
		      ...
		      require('phpmailer/src/Exception.php');
                ...
                $mail->isHTML(true);
                $mail->Subject = "Quotation from BillingBizz";
                $mail->Body = "Hi, Please Find Attached Quotation";
  ...
  if($_GET['ACTION']=='DELETE'){
      $sql= "DELETE from quotation where SID ='{$_GET["id"]}'";
