<?php 
 ...
 if($res1->num_rows>0){
  ...
  $sql="select * from deliverychallan where SID='{$_GET["id"]}'";
  $res=$con->query($sql);
  if($res->num_rows>0){
	  $row=$res->fetch_assoc();
  ...
  class PDF extends FPDF
  {
    ...
    function body($registered_users,$info,$products_info){
        ...
        $this->Cell(50,25,"Delivery Challan",0,1);
      ...
      $this->SetY(55);
      $this->SetX(-60);
      $this->Cell(50,7,"Invoice No : ".$info["challan_no"]);
      
      //Display Invoice date
      $this->SetY(63);
      $this->SetX(-60);
      $this->Cell(50,7,"Invoice Date : ".$info["challan_date"]);
...
//   $pdf->Output('deliverychallan.pdf','I');
  
  $filepath= "uploads/";
  $filename= $info["customer"].'_deliverychallan.pdf';
  
  if($_GET['ACTION']=='EMAIL'){
    
    header("location: records-delivery.php");
    ...
    $pdf->Output($filepath.$filename,'F');
		      ...
		      require('phpmailer/src/Exception.php');
                ...
                $mail->isHTML(true);
                $mail->Subject = "Delivery Challan from Invoice Maker";
                $mail->Body = "Hi, Please Find Attached Delivery Challan";
  ...
  if($_GET['ACTION']=='DELETE'){
      $sql= "DELETE from deliverychallan where SID ='{$_GET["id"]}'";
